...
#Zeus
wget -q -O - https://zeustracker.abuse.ch/blocklist.php?download=ipblocklist --no-check-certificate | egrep -v '(^[[:space:]]*/|^[[:space:]]*#|^[[:space:]]*$)|/[0-9]|\:|/g' | awk -F" " '{print $1}' | sort -u >> $path/$ts_'ip_blacklist.txt'

#Spyeye
wget -q -O - https://spyeyetracker.abuse.ch/blocklist.php?download=ipblocklist --no-check-certificate | egrep -v '(^[[:space:]]*/|^[[:space:]]*#|^[[:space:]]*$)|/[0-9]|\:|/g' | awk -F" " '{print $1}' | sort -u >> $path/$ts_'ip_blacklist.txt'

#OpenBl
wget -q -O - http://www.openbl.org/lists/base.txt | egrep -v '(^[[:space:]]*/|^[[:space:]]*#|^[[:space:]]*$)|/[0-9]|\:|/g' | awk -F" " '{print $1}' | sort -u >> $path/$ts_'ip_blacklist.txt'
...
#Malc0de
wget -q -O - http://malc0de.com/bl/IP_Blacklist.txt | egrep -v '(^[[:space:]]*/|^[[:space:]]*#|^[[:space:]]*$)|/[0-9]|\:|/g' | awk -F" " '{print $1}' | sort -u >> $path/$ts'_ip_blacklist.txt'
